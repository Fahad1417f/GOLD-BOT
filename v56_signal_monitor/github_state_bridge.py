...
REPO=os.getenv("GOLDBOT_REPO","Fahad1417f/GOLD-BOT"); BRANCH=os.getenv("GOLDBOT_BRANCH","main")
STATE_PATH=os.getenv("GOLDBOT_STATE_PATH","website_state.json"); TOKEN=os.getenv("GITHUB_TOKEN","").strip()
LOG=Path(os.getenv("GOLDBOT_MONITOR_LOG","v56_monitor.log"))
INTERVAL=float(os.getenv("GOLDBOT_PUBLISH_INTERVAL","10")); MIN_PUBLISH=float(os.getenv("GOLDBOT_MIN_PUBLISH_SECONDS","20"))
...
def latest(t,pattern,default="unknown"):
    m=list(re.finditer(pattern,t,re.I|re.M)); return m[-1].group(1).strip() if m else default
...
def parse_log(t):
    ...
    if s["direction"]=="neutral":
        if s["gravity"]["4h"]==s["gravity"]["1h"] and s["gravity"]["4h"] in ("long","short"):s["direction"]=s["gravity"]["4h"]
    ...
    while True:
        try:
            if LOG.exists():
                ...
                if t.strip():publish(parse_log(t))
