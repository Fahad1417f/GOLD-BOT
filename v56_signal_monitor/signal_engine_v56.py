...
def promote(analysis:dict[str,dict],timing:dict[str,Any]|None=None)->Signal:
    ...
    ldir=_dir_from_bias(leader.get('active_kfoo')) or _dir_from_bias(linfo.get('kfoo_table_direction',{}).get('bias')) or _dir_from_bias(linfo.get('direction'))
    ...
    gravity_aligned=bool(g4 and g1 and g4==g1 and s4 and s1); leader_aligned=bool(gravity_aligned and ldir==g4)
    kfoo_quality=leader_table_ok and tf_pct>=62.5 and ind_pct>=75.0 and active15==ldir
    ...
    if not gravity_aligned or not leader_aligned or not kfoo_quality:
        return Signal('WAIT',g4 or ldir or 'neutral',0.40 if gravity_aligned else 0.20,reasons,setup_ready=False,entry_ready=False,timestamp_utc=datetime.now(timezone.utc).isoformat())
