...
echo READ_ONLY=YES
echo EXECUTION=OFF
echo CYCLE_TIMEOUT=%GOLDBOT_VISION_CYCLE_TIMEOUT%s
echo VISION_TRACE=ON
echo.
...
start "GOLD-BOT VISION" cmd /k "cd /d "%~dp0" && set GOLDBOT_VISION_INTERVAL_SECONDS=%GOLDBOT_VISION_INTERVAL_SECONDS% && set GOLDBOT_VISION_CANDLE_LIMIT=%GOLDBOT_VISION_CANDLE_LIMIT% && set GOLDBOT_VISION_CYCLE_TIMEOUT=%GOLDBOT_VISION_CYCLE_TIMEOUT% && set GOLDBOT_VISION_TRACE=%GOLDBOT_VISION_TRACE% && set GOLDBOT_VISION_OUTPUT_DIR=artifacts\vision && set GOLDBOT_REAL_TRADING=OFF && set GOLDBOT_DEMO_TRADING=OFF && set GOLDBOT_FAST_TRADE=OFF && "%PYTHON_EXE%" -u -m kfoo_linked_work.vision_cycle_watchdog_v1"
