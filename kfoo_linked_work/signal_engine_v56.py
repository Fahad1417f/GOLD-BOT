...
def _hns_gate(verified_hns_mtf: dict | None, gravity: str | None) -> tuple[bool, list[str]]:
    """Return (blocked, reasons) for verified MTF H&S evidence.

    H&S is additive confirmation: a detected-but-unconfirmed pattern never blocks
    an otherwise valid setup. A verified confirmed pattern that conflicts with
    higher-timeframe gravity blocks promotion (fail-closed).
    """
