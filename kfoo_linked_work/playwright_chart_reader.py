...
class ChartRead:
    ...
    diagnostics:dict|None=None; plot_rect:tuple[int,int,int,int]|None=None
...
class PlaywrightChartReader:
    ...
    def read(self):
        ...
        try:
            ...
            symbol,timeframe=self._parse_metadata(title,text,self.page); candles=self._read_verified_candles(); plot=self._read_plot_rect()
            verified=bool(symbol and timeframe); reason="VERIFIED_CANDLES_READ" if candles else ("IDENTITY_VERIFIED" if verified else "IDENTITY_NOT_VERIFIED")
    ...
    def capture(self,name="tradingview_live.png"):
        ...
        self.screenshot_dir.mkdir(parents=True,exist_ok=True); path=self.screenshot_dir/name; self.page.screenshot(path=str(path),full_page=False); return str(path)
    ...
    def _normalize_timeframe(value):
        ...
        if m:return m.group(1)+"h"
        m=re.fullmatch(r"(1|3|5|15|30|45)\s*(?:دقيقة|دقائق)",s)
        if m:return m.group(1)+"m"
        m=re.fullmatch(r"(1|2|4|6|12)\s*(?:ساعة|ساعات)",s)
        if m:return m.group(1)+"h"
        return {"1":"1m","3":"3m","5":"5m","15":"15m","30":"30m","45":"45m","60":"1h","120":"2h","240":"4h","360":"6h","720":"12h","1d":"1d","1w":"1w"}.get(s.lower())
    @staticmethod
