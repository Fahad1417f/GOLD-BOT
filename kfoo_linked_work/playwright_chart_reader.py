...
class PlaywrightChartReader:
    ...
    def read(self):
        ...
        try:
            ...
            return ChartRead(connected=True,verified=verified,
                ...
                candles=candles,reason="VERIFIED_CANDLES_READ" if candles else ("IDENTITY_VERIFIED" if verified else "IDENTITY_NOT_VERIFIED"))
    ...
    def _normalize_timeframe(value):
        ...
        if m: return f"{m.group(1)}m" if m else None
    ...
    def _parse_metadata(title,text,page=None):
        ...
        # Deliberately do not infer timeframe from the full visible menu.
    ...
    def _parse_title(title):
        ...
        if re.search(r"\bXAU\s*/?\s*USD\b|\bXAUUSD\b",u):return "XAU/USD",None
        m=re.search(r"\b([A-Z]{2,10})\s*/\s*([A-Z]{2,10})\b",u)
        return (f"{m.group(1)}/{m.group(2)}",None) if m else (None,None)
