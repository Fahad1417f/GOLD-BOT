...
def test_duplicate_overlay_fragments_are_collapsed_to_one_x_series(tmp_path):
    p = tmp_path / "chart_with_duplicate_fragments.png"
    im = Image.new("RGB", (900, 500), (15, 15, 15))
    d = ImageDraw.Draw(im)

    # Six genuine candle columns with the same regular pitch.
    for i, x in enumerate((180, 212, 244, 276, 308, 340)):
        top = 180 - (i % 2) * 10
        bottom = top + 26
        color = (3, 207, 112) if i % 2 == 0 else (241, 1, 49)
        d.rectangle((x - 4, top, x + 4, bottom), fill=color)
        d.line((x, top - 12, x, top), fill=color, width=2)
        d.line((x, bottom, x, bottom + 12), fill=color, width=2)
        # Nearby indicator/annotation fragments at the same candle x should
        # not become separate candles.
        d.rectangle((x + 7, 300, x + 10, 325), fill=color)

    im.save(p)
    r = detect_candles(p)
    assert r.verified
    assert 3 <= len(r.candles) <= 6
    xs = [c.x for c in r.candles]
    assert all(xs[i + 1] - xs[i] >= 16 for i in range(len(xs) - 1))
