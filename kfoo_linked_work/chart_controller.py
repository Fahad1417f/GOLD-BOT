...
class ChartController:
    def __init__(self, cdp_url=None, chart_point=(700,450), verify_delay=0.8):
        ...
        self.chart_point=tuple(chart_point)
    ...
    def connect(self)->ChartState:
        ...
        try:
            ...
            return ChartState(reason=f"CDP_CONNECT_FAILED:{type(exc).__name__}:{exc}")
    ...
    def state(self)->ChartState:
        ...
        try:
            ...
            tf=self._timeframe_from_title(title); symbol=self._symbol_from_title(title)
            return ChartState(True,title,url,tf,symbol,bool(tf or symbol),"TITLE_READ")
        except Exception as exc:
            return ChartState(reason=f"STATE_READ_FAILED:{type(exc).__name__}:{exc}")
    ...
    def _symbol_from_title(title):
        m=re.search(r"\b([A-Z]{2,10})[/:-]([A-Z]{2,10})\b",(title or "").upper())
    ...
    def set_timeframe(self, timeframe):
        ...
        # Safety: this controller intentionally does not click UI elements or navigate.
        # A higher-level verified adapter must implement chart-only timeframe actions.
