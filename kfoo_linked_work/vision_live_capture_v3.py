"""Vision live capture v3: deep DOM/shadow-root price-scale discovery.

Wraps v1 and changes only the price-scale evidence search. No anchors are
fabricated; OHLC and execution remain fail-closed.
"""
...
def _read_price_scale_anchors(reader, plot_rect, screenshot_path):
    ...
    try:
                    ...
                    for(const el of elements){
                        ...
                        if(r.width&&r.height&&r.bottom>=y0&&r.top<=y1){
                            ...
                            if(right||left){
                                ...
                                for(const t of [...new Set(values.map(v=>String(v||'').trim()).filter(Boolean))]) emit(t,r,null,'element');
                            }
                        }
                        if(el.shadowRoot)visit(el.shadowRoot);
                    }
                    const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);
                    for(let n=walker.nextNode();n;){
                        ...
                        try{
                            ...
                            const right=r.left>=x1-24&&r.left<=x1+420; const left=r.right>=x0-420&&r.right<=x0+24;
        ...
        for c in sorted(candidates,key=lambda v:(v["side"] or "",v["y"],v["x"],v["source"] or "")):
            if any(abs(c["y"]-d["y"])<=2 and c["price"]==d["price"] and c["side"]==d["side"] for d in dedup): continue
            dedup.append(c)
        pair_pool=[]
        for side in ("right","left"):
            ...
            for a,b in zip(items,items[1:]):
                ...
                if dy>=8 and dp<0: pair_pool.append((dy,abs(dp),a,b))
        selected=[]
        if pair_pool:
            ...
            for c in dedup:
                ...
                if any(abs(c["y"]-s["y"])<=4 and c["price"]==s["price"] for s in selected): continue
                if all((c["y"]-s["y"])*(c["price"]-s["price"])<0 for s in selected): selected.append(c)
        anchors=[ScaleAnchor(c["y"],c["price"]) for c in selected[:12]]
        return anchors,{"reason":"VISIBLE_SCALE_LABEL_PAIRS_FOUND" if len(anchors)>=2 else "NO_VERIFIED_SCALE_LABEL_PAIR","candidate_count":len(candidates),"distinct_count":len(anchors),"scan_sides":["right","left"],"sources":sorted(set(c.get("source") for c in dedup if c.get("source"))),"candidates":dedup[:40]}
